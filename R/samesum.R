...
#'
#' @param scale Aim to give each sample close to this value of scale. The default value of 2 should be reasonable for count data, but see discussion below.
#'
#' @param target_sum Aim to give each sample exactly this sum. \code{scale} is ignored if this parameter is used. This could be used to provide a transformation that is consistent across datasets.
#'
...
#'
#' @export
samesum_log2_norm <- function(x, scale=2, target_sum=NA, tol=1e-9, verbose=FALSE) {
...
#' @export
samesum_log2_cpm <- function(x, scale=2, target_sum=NA, tol=1e-9, verbose=FALSE) {
